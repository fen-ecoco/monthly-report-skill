...
# uploads the generated report as a draft to the ECOCO Report system.
...
try {
    ...
    if ($genExitCode -eq 0) {
        Add-Content -Path $LogFile -Value "月報產出成功，接著上傳草稿..." -Encoding UTF8
        if (Test-Path $UploadScript) {
            & $pythonExe $UploadScript 2>&1 | Tee-Object -FilePath $LogFile -Append
            $uploadExitCode = $LASTEXITCODE
            if ($uploadExitCode -eq 0) {
                Add-Content -Path $LogFile -Value "草稿上傳成功。" -Encoding UTF8
            } elseif ($uploadExitCode -eq 3) {
                Add-Content -Path $LogFile -Value "提示：既有報表非草稿狀態（審核中／已核准），已跳過覆蓋，屬正常保護機制，非錯誤，請自行至系統網頁確認。" -Encoding UTF8
            } else {
