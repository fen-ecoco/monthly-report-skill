...
├── upload_monthly_report.py          ← 月報自動上傳至 ecowork（含真實 API Token，勿上傳 GitHub）
...
## 七、月報自動上傳至 ecowork（2026-09 起）

月報產出成功後，`Run-MonthlyReport.ps1` 會自動接著呼叫 `upload_monthly_report.py`，
把月報 MD 上傳到 **ECOwork 彙報中心**（`https://space.ecocogroup.com/#reports`），**直接送出審核**（不再只是存草稿）。

> 2026-09 之前，本系統上傳目標是舊版「週/月報系統」（`report-a.ecocogroup.com`），單一 POST 即完成上傳並存為草稿。
> 2026-09 起遷移到 ecowork 新平台，因新 API 規定必須依序呼叫多個端點，流程改為四步驟，且會直接送審而非停在草稿。

### 檔案與設定

`upload_monthly_report.py` 裡的 `CONFIG` 需要填入 `api_token`——**ecowork 個人 API Token（`ecw_` 開頭）**，
需要同時具備 **`reports:read`**（查期別）與 **`reports:write`**（建草稿/填內容/送出）兩個權限，缺一都會失敗。
這組 token **必須跟週報自動化用的 token 分開申請，不可共用**（兩邊各自的排程互不影響）。
本檔案含真實 Token，切勿上傳到 GitHub 或任何版本控制系統，`.gitignore` 已將它排除；
repo 裡只放 `upload_monthly_report.example.py` 範本版（Token 為佔位符）。

### 運作邏輯（四步驟）

1. `GET /api/reports/periods?type=monthly` — 取得「本期」的期別代號（如 `2026-09`）。回應欄位名稱官方文件未完全公開，程式會嘗試比對幾種常見寫法，判斷不出來時會印出原始回應內容並退回用系統日期推算
2. `POST /api/reports/drafts` — 建立或取得既有草稿（`reportType: "monthly"`、`templateMode: "free"`）；若該期別**已有送出的報表** → API 回傳 409，程式印出通知並中止，**不會覆蓋**
3. `PATCH /api/reports/{id}` — 填入內容。`content` 物件固定用鍵 **`"報表內容"`**（自由撰寫模式沒有範本可套欄位標籤，此鍵名稱已用瀏覽器 Network 實測確認），並附上 `aiContribution`（AI協作說明，固定文字：「使用內部規則式腳本彙整週報產生月報，未使用付費AI API」；**以 API Token 呼叫時此欄位為伺服器端必填，不可空白**）
4. `POST /api/reports/{id}/submit` — 送出審核。**送出後內容無法再修改**，撤回只能到網頁操作，請確認內容無誤再排程自動送出

### 特別注意：Cloudflare 與回應解壓縮

ecowork 一樣掛 Cloudflare 防護，request header 需要帶完整瀏覽器特徵（`User-Agent`、`Accept-Language`、`Sec-Ch-Ua` 等），
否則會被擋下 `403 Error 1010`（訊息：the site owner has blocked access based on your browser's signature）。
另外因為 header 宣告了 `Accept-Encoding: gzip, deflate`，Python 內建的 `urllib` **不會自動解壓縮回應內容**，
若直接 `.decode("utf-8")` 會拋出 `UnicodeDecodeError`；程式內建 `decode_body()` 依回應的 `Content-Encoding` 標頭手動解壓縮處理。
...
重複執行是安全的：同期別已建過草稿會直接取用繼續填內容；已送審過的期別，`drafts` 那步會被 409 擋下，不會重複送出。
...
| `upload_monthly_report.py` | 0 | 已建草稿、填內容、送出審核成功 |
| `upload_monthly_report.py` | 3 | 主動保護不覆蓋（該期別已有送出的報表），正常情況，非錯誤 |
| `upload_monthly_report.py` | 1 | 真正的上傳錯誤（如 Token 權限不足、Cloudflare 擋下、期別判斷失敗、送出失敗） |
...
| 排程沒有執行 | 開啟工作排程器（`taskschd.msc`）找到 `ECOCO-MonthlyReport`，檢查「歷程記錄」分頁；也可用 `Get-ScheduledTaskInfo -TaskName "ECOCO-MonthlyReport"` 查 `LastTaskResult`（0=成功） |
| 電腦排程觸發當下是關機的 | 預設錯過就不會補跑，除非在工作排程器該任務的「設定」分頁勾選「如果錯過排定的啟動時間，盡快執行工作」 |
| 想立即手動觸發排程測試 | `Start-ScheduledTask -TaskName "ECOCO-MonthlyReport"` |
| 上傳出現 `403 Error 1010` | Cloudflare 擋下，檢查 `build_headers()` 裡瀏覽器偽裝標頭是否完整（User-Agent、Sec-Ch-Ua 等） |
| 上傳出現 `UnicodeDecodeError` | 回應內容是壓縮過的二進位資料，確認 `decode_body()` 有依 `Content-Encoding` 解壓縮 |
| 上傳出現 `API Token 缺少權限：reports:read` 或 `reports:write` | 這組 PAT 缺少對應 scope，須回 ecowork 後台補授權，不能只靠換 token 解決 |
...
| 月報上傳失敗 | 確認 `upload_monthly_report.py` 裡的 `api_token` 是否正確、是否已過期、scope 是否齊全（`reports:read` + `reports:write`） |

## 九、費用

完全免費。整個流程（判斷日期、下載政府行事曆、解析週報、產出月報、上傳送審）都在你自己的電腦上執行，
不呼叫任何付費 AI API（ecowork 本身屬於公司內部系統，非本文所指的付費 AI API）。
